    ...
    def apply_shift(self, shift):
        ...
        for letter in  self.message_text:
            if (letter in shift_dict.keys()):
                message_text_encrypted += shift_dict[letter]
            else:
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(1,27):
            tmp_sum = 0
            message_text_decrypted = self.apply_shift(26 - s)
            for word in message_text_decrypted.split():
                if (is_word(self.valid_words, word)):
                    tmp_sum += 1
            if (tmp_sum > max_realWords):
                max_realWords = tmp_sum
                best = (26 - s, message_text_decrypted)
        return best
...
#    print('After change: ', plaintext.get_message_text_encrypted())
#    #print(plaintext.get_encryption_dict())


   #Example test case (CiphertextMessage)
   ciphertext = CiphertextMessage('jgnnq cuong')
